...
Exact CLI columns: `timestamp,sales,<channels...>[,controls...]`. All configs require `source` and `units`; `outcome_due` is recorded for future scoring. Supported method controls: `channels, controls, decay_grid, saturation, alpha, origins, windows, prior_mean, prior_sd, noise_sd, reallocation_total, horizon, season, frequency, as_of, seed, decay_a, decay_b, half_a, half_b, initial_a, initial_b`. Unknown config keys are rejected.
