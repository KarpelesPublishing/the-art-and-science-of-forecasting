...
SKIP_PARTS = {'.venv', '.venv-best', '__pycache__', '.pytest_cache', 'legacy-run-journals', 'checkpoints', 'batch-benchmark', 'layout-pages', 'chart-sheets', 'applied-runs', '.ipynb_checkpoints', '.DS_Store'}
...
def wanted(rel):
    ...
    if rel.parts[0] == 'companion':
        ...
        if inner.startswith('data/benchmarks/'):
            return False                                  # benchmark data is fetched, never bundled
